...
export class Monitor implements IMonitor {
	...
	public createProgress(releaseJob: IReleaseJob): IReleaseProgress {
		...
		for (const stage of releaseJob.stages) {
			const releaseStage: ReleaseEnvironment = releaseJob.release.environments!.find((i) => i.name === stage)!
	...
	public updateReleaseProgress(releaseProgress: IReleaseProgress): void {
		...
		const completedStages: string[] = releaseProgress.stages.filter((stage) => this.isStageCompleted(stage)).map((stage) => stage.name)

		const activeStages: string[] = releaseProgress.stages.filter((stage) => this.isStageActive(stage)).map((stage) => stage.name)
		...
		if (completed) {
			...
			const failed: boolean =
				releaseProgress.stages.filter((i) => i.status === EnvironmentStatus.Rejected || i.status === EnvironmentStatus.Canceled).length > 0

			if (failed) {
				releaseProgress.status = ReleaseStatus.Failed
			} else {
				// Get partially succeeded stages
				const partiallySucceeded: boolean = releaseProgress.stages.filter((i) => i.status === EnvironmentStatus.PartiallySucceeded).length > 0
	...
	public updateStageProgress(stageProgress: IStageProgress, stageStatus: ReleaseEnvironment): void {
		...
		const currentAttempt: DeploymentAttempt = stageStatus.deploySteps!.sort((left, right) => left.deploymentId! - right.deploymentId!).reverse()[0]
		...
		stageProgress.duration = stageStatus.timeToDeploy ? stageStatus.timeToDeploy.toLocaleString() : undefined
	}
	...
	public getActiveStages(releaseProgress: IReleaseProgress): IStageProgress[] {
		...
		const activeStages: IStageProgress[] = releaseProgress.stages.filter((stage) => !this.isStageCompleted(stage))
	...
	public getPendingStages(releaseProgress: IReleaseProgress): IStageProgress[] {
		...
		const pendingStages: IStageProgress[] = releaseProgress.stages.filter((stage) => this.isStagePending(stage))
	...
	public isStagePending(stageProgress: IStageProgress): boolean {
		...
		const status: boolean = stageProgress.status !== EnvironmentStatus.Queued && stageProgress.status !== EnvironmentStatus.InProgress
