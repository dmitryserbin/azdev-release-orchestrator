...
export class ProgressReporter implements IProgressReporter {
	...
	public logRun(run: IRun): void {
		...
		const stages: string[] = run.stages.map((stage) => (stage.target === true ? `${stage.name}*` : stage.name))

		const releaseDate: Date | null = run.build.queueTime ? new Date(run.build.queueTime!) : null
	...
	public logParameters(parameters: IBuildParameters): void {
		const table: Table = this.newTable(["Name", "Value"])
		...
		for (const parameter of Object.keys(parameters)) {
			...
			const result: any[] = [parameter, value]
	...
	public logFilters(filters: IFilters, strategy: Strategy): void {
		...
		switch (strategy) {
			case Strategy.New: {
				...
				const pipelineResources: string[] = Object.keys(filters.pipelineResources).map((i) => `${i}|${filters.pipelineResources[i]}`)

				const repositoryResources: string[] = Object.keys(filters.repositoryResources).map((i) => `${i}|${filters.repositoryResources[i]}`)
	...
	public logStageProgress(stage: IBuildStage): void {
		const table: Table = this.newTable(["Agent", "Job", "Task", "Result", "Duration"])

		for (const job of stage.jobs) {
			for (const task of job.tasks) {
				const duration: string | null =
					task.startTime && task.finishTime ? Moment.duration(new Date(task.startTime).getTime() - new Date(task.finishTime).getTime()).humanize() : null
	...
	public logStagesProgress(stages: IBuildStage[]): void {
		const table: Table = this.newTable(["Stage", "Jobs", "Tasks", "Attempt", "Checkpoint", "Result", "Duration"])

		for (const stage of stages) {
			const tasksCount: number = stage.jobs.map((job) => job.tasks.length).reduce((a, b) => a + b, 0)

			const duration: string | null =
				stage.startTime && stage.finishTime ? Moment.duration(new Date(stage.startTime).getTime() - new Date(stage.finishTime).getTime()).humanize() : null
	...
	public logRunProgress(runProgress: IRunProgress): void {
		const table: Table = this.newTable(["ID", "Build", "Stages", "Result", "Summary"])

		const stages: string[] = runProgress.stages.map((stage) => stage.name)
