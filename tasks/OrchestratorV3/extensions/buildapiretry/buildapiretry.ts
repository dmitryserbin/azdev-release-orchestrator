...
export class BuildApiRetry implements IBuildApiRetry {
	...
	public async getBuildTimeline(project: string, buildId: number, timelineId?: string, changeId?: number, planId?: string): Promise<Timeline> {
		return await this.buildApi.getBuildTimeline(project, buildId, timelineId, changeId, planId)
	}
	...
	public async queueBuild(
		...
		return await this.buildApi.queueBuild(build, project, ignoreWarnings, checkInTicket, sourceBuildId, definitionId)
	}

	@Retryable()
	public async updateBuild(build: Build, project: string, buildId: number, retry?: boolean): Promise<Build> {
		return await this.buildApi.updateBuild(build, project, buildId, retry)
	}
	...
	public async getDefinition(
		...
		return await this.buildApi.getDefinition(project, definitionId, revision, minMetricsTime, propertyFilters, includeLatestBuilds)
	}
